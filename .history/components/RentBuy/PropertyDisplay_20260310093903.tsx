...
const parseCommaParam = (raw: string | null): string[] => {
  if (!raw) return [];
  return raw
    .split(",")
    .map((s) => s.trim())
...
const buildLabelMap = () => {
  ...
  propertytypes?.forEach((pt) => {
    ...
    const label = (currentLocale === "en" ? pt.nameen : pt.namefr) ?? pt.namefr ?? slug;
  ...
  transactions?.forEach((tr) => {
    ...
    const label = (currentLocale === "en" ? tr.nameen : tr.namefr) ?? tr.namefr ?? slug;
    if (slug) map.set(String(slug), label);
  });
...
const buildPrefilledMessage = (): string => {
  ...
  // read raw params
  const typeRaw = parseCommaParam(searchParams?.get("type"));
  const transactionRaw = parseCommaParam(searchParams?.get("transaction"));
  const cityRaw = parseCommaParam(searchParams?.get("city"));
  const priceRaw = parseCommaParam(searchParams?.get("price"));
  const areaRaw = parseCommaParam(searchParams?.get("area"));
