...
const PropertyCard = ({ data }: { data: PropertyProps }) => {
  ...
  return (
    <Link className={styles.property__card} href={`/${currentLocale}/services/${data.slug.current}`}>
      <div className={styles.property__top}>
        <div className={styles.p__image}>
          <Image unoptimized placeholder="blur" fill quality={100} src={urlFor(data.mainimage).width(1920).url() || urlFor(data.mainimage).url()} alt={data.mainimage.alt} />
        </div>
      </div>
            ...
            {data.transaction.slug === "rent" && data.rentpricing && (
              <span>
                {data.rentpricing === "perday"
                  ...
                  data.rentpricing === "permonth" ? t("ServicesPage:permonth")
                  : t("ServicesPage:peryear")}
              </span>
            )}
          </div>
          {"|"}
          <span className={styles.location}>{data.quarter}, {data.city.cityname}</span>
        </div>
        <div className={styles.p__stats}>
          {data.propertytype && <span>{currentLocale==="en" ? data.propertytype.nameen : data.propertytype.namefr}</span>}
          {data.room && (
            <span>
