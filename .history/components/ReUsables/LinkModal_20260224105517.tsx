...
const LinkModal = ({
  ...
  // Fermer le modal quand on clique en dehors useEffect(() => { const handleOutsideClick = (e: MouseEvent) => { if (!activeModal) return; const el = targetRef?.current; if (el && !el.contains(e.target as Node)) { setActiveModal(false); } }; document.addEventListener("pointerdown", handleOutsideClick); return () => { document.removeEventListener("pointerdown", handleOutsideClick); }; }, [activeModal, targetRef]);
