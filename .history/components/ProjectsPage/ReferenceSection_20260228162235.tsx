...
const ReferenceSection = () => {
  ...
  // find the matching project const selectedData = refContent.find((item) => item.slug === ref);
  return (
    <>
        ...
        <div className={`container ${styles.ref__container}`}>
          {refContent.map((data, i) => (
            <div className={styles.ref__box} key={i}>
              ...
              <RefImage alt={data.name} src={data.image} ref={data.slug} />
            </div>
          ))}
        </div>
      </div>
      {ref && <ReferenceDetails data={refContent[0]} />}
    </>
  );
};
