...
const ReferenceSection = () => {
  ...
  const router = useRouter(); const searchParams = useSearchParams(); const handleClick = () => { const params = new URLSearchParams(searchParams.toString()); params.set("ref", "myValue"); // set or update the param router.push(`?${params.toString()}`); // navigate with new query string };
        ...
        <div className={`container ${styles.ref__container}`}>
          {refContent.map((data, i) => (
            <div className={styles.ref__box} key={i}>
              ...
              <RefImage alt={data.name} src={data.image} />
            </div>
          ))}
        </div>
      </div>
      <ReferenceDetails data={refContent[0]} />
    </>
  );
};
