import React from "react";
import { StaticImageData } from "next/image";
import { ReferenceAnim } from "@/animation";
import { useTranslation } from "react-i18next";
import { motion } from "framer-motion";
import Image from "next/image";
import { AppRouterInstance } from "next/dist/shared/lib/app-router-context.shared-runtime";
import type { ReadonlyURLSearchParams } from "next/navigation";
import HiglightedSpan from "../ReUsables/HiglightedSpan";
import Plus from "@/utils/Icons/Plus";
import VideoJS from "../ReUsables/VideoJS";
import styles from "../../styles/ProjectsPage/refdetails.module.scss";

interface RefProps {
  image: StaticImageData;
  slug: string;
  name: string;
  aboutproject?: string[];
  content: {
    name: string;
    info: string | number;
  }[];
  gallery?: {
    type: string;
    src: StaticImageData | string;
  }[];
}

const ReferenceDetails = ({
  data,
  router,
  searchParams,
}: {
  data: RefProps;
  router: AppRouterInstance;
  searchParams: ReadonlyURLSearchParams;
}) => {
  const { t } = useTranslation();

  const handleClick = () => {
    const params = new URLSearchParams(searchParams.toString());
    params.delete("ref"); // set or update the param
    router.push(`?${params.toString()}`, { scroll: false }); // navigate with new query string
  };

  const ref = searchParams.get("ref");

  return (
    <motion.div
      className={styles.refdetails}
      variants={ReferenceAnim}
      animate={ref && data ? "entry" : "initial"}
      exit="exit"
    >
      <div className={styles.ref__wrapper} data-lenis-prevent>
        <div className={styles.ref__container}>
          <div className={styles.ref__top}>
            <h3 className={styles.rbl__name}>{data.name}</h3>
            <div className={styles.rbl__info}>
              {data.content.map((info, i) => (
                <div className={styles.rinfo} key={i}>
                  <span className={styles.rr}>{info.name}</span> :{" "}
                  <span className={styles.info}>{info.info}</span>
                </div>
              ))}
            </div>
          </div>
          <div className={styles.main__image}>
            <Image
              placeholder="blur"
              fill
              quality={100}
              alt={data.name}
              src={data.image}
            />
          </div>
          {data.aboutproject && (
            <div className={styles.about__project}>
              <HiglightedSpan text={t("ProjectsPage:aboutpro")} />
              <div className={styles.about__data}>
                {data.aboutproject.map((text, i) => (
                  <p key={i}>{text}</p>
                ))}
              </div>
            </div>
          )}

          {data.gallery && (
            <div className={styles.project__gallery}>
              {data.gallery.map((gal, i) => {
                if (gal.type === "img") {
                  return (
                    <div className={styles.gallery__images} key={i}>
                      <Image placeholder="blur" fill quality={100} src={gal.src} alt={data.name} />
                    </div>
                  );
                } else {
                  const videoJsOptions = {
                    autoplay: true,
                    muted: true,
                    loop: true,
                    controls: false,
                    playsinline: true,
                    responsive: true,
                    fluid: true,
                    sources: [
                      {
                        src: gal.src, // dynamic per item
                        type: "video/mp4",
                      },
                    ],
                  };

                  return (
                    <div key={i} className={styles.video__wrapper}>
                      <VideoJS options={videoJsOptions} />
                    </div>
                  );
                }
              })}
            </div>
          )}
        </div>
      </div>
      <div className={styles.cancel} onClick={handleClick}>
        <span className={styles.icon}>
          <Plus />
        </span>
      </div>
    </motion.div>
  );
};

export default ReferenceDetails;
