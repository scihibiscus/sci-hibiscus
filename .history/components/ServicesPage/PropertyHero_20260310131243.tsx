...
const PropertyHero = ({ property }: { property: PropertyProps }) => {
  ...
  function getTransactionType () {
    ...
    if (property.transaction.slug.current==="rent"){
      currentLocale==="en" ? transaction="For Rent" : transaction = "À Louer"
    } else {
      currentLocale==="en" ? transaction="For Sale" : transaction = "À Vendre"
    }
    return transaction
  }
          ...
          <div className={styles.hs__content}>
            <div className={styles.hsc__left}>
              ...
              <div className={styles.prop__charac}>
                ...
                {property.room && (
                  <span>
                    {property.room} {t("ServicesPage:room")}
                  </span>
                )}
                {property.bath && (
                  <span>
                    {property.bath} {t("ServicesPage:bath")}
                  </span>
                )}
                {property.parlour && (
                  <span>
                    {property.parlour} {t("ServicesPage:living")}
                  </span>
                )}
                {property.area && <span>{keyData.area} m²</span>}
              </div>
            </div>
